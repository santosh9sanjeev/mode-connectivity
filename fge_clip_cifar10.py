...
# checkpoint = torch.load('/home/santoshsanjeev/model_soups/dnn-mode-connectivity/cnn_v1/model_20.pth')
# model.load_state_dict(checkpoint['model_state_dict'])
pre_model, _ = clip.load('ViT-B/32')
checkpoint1 = torch.load('/share/nvmedata/santosh/model_soups/models_model_soups/cifar10_model_soups_models/checkpoint_10.1.pt')
...
normalize = True
clf = train_utils.zeroshot_classifier(pre_model, classnames, template, 'cuda')
...
# test_res = utils.test(loaders['test'], model1, criterion)
...
ensemble_size = 0
predictions_sum = np.zeros((len(loaders['val'].dataset), num_classes))
...
for epoch in range(args.epochs):
    ...
    predictions, targets = utils.predictions(loaders['val'], model)
    ens_acc = None
